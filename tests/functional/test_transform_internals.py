...
Note on vmap: `pxf.vmap` is broken on jax >= 0.4.34 (BUGS.md#4), so every test in
that section fails before it reaches the behaviour it describes. They are written
out anyway — they are the specification the fix has to satisfy.
"""
...
# vmap #################################################################################
#
# `Vmap._t` calls `_process_mask` without an `rkg_mask`, so `mask["__RKG"]` is None
# while the kwargs hold a real `RandomKeyGenerator`; jax >= 0.4.34 rejects the
# mismatch before the transform runs (BUGS.md#4). Every test below is marked
# accordingly and states what the machinery is supposed to do.
...
@pytest.mark.bug("BUGS.md#4: pxf.vmap broken on jax>=0.4.34 (__RKG mask is None)")
